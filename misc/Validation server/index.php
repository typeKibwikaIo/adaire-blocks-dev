 ...
 */
function makeLicenseRequest( $endpoint, $method = 'GET', $data = array() ) {
	global $config;
...
function validateLicense( $license_key ) {
	logServerOperation( 'validate', array( 'license_key' => $license_key ) );
...
function activateLicense( $license_key ) {
	logServerOperation( 'activate', array( 'license_key' => $license_key ) );
	...
	if ( is_array( $api_response ) ) {
		...
		writeToDebugLog(
			'Adaire Blocks Validation Server: Token extraction - ' .
...
function healthCheck() {
	logServerOperation( 'health_check', array() );
	...
	if ( function_exists( 'curl_init' ) ) {
		...
		$server_info['license_api_connectivity'] = array(
