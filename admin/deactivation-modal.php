<?php
/**
 * Deactivation feedback modal shown on the Plugins screen.
 *
...
/**
 * Controls the "Before you go…" deactivation feedback modal: rendering,
 * asset loading, and the AJAX handler that receives the submitted feedback.
 */
class Adaire_Deactivation_Modal
{
     ...
     * Singleton instance.
     *
     * @var Adaire_Deactivation_Modal|null
     */
    private static $instance = null;
    private const DEFAULT_FEEDBACK_EMAIL = 'support@adaire.com';

    /**
     * Singleton access for the modal controller.
     *
     * @return Adaire_Deactivation_Modal
     */
    ...
    /**
     * Register hooks for assets, UI, and AJAX.
     */
    ...
    /**
     * Load assets for the plugins page only.
     *
     * @param string $admin_hook Current admin page hook suffix.
     */
    public function load_modal_assets($admin_hook)
    {
        if ('plugins.php' !== $admin_hook) {
            return;
        }
    ...
    /**
     * Render the feedback modal markup on the plugins page.
     */
    ...
    /**
     * Receive feedback, send email, and log the result.
     */
    ...
    /**
     * Sanitize incoming feedback fields.
     *
     * @param array $raw_feedback_input Raw $_POST data.
     * @return array
     */
     ...
     *
     * @param array $feedback Sanitized feedback fields.
     * @return string
     */
    private function build_feedback_message(array $feedback)
    {
        $reason_text = $feedback['reason'] ? $feedback['reason'] : 'Not provided';
        $details_text = $feedback['details'] ? $feedback['details'] : 'Not provided';
        $email_text = $feedback['email'] ? $feedback['email'] : 'Not provided';
    ...
    /**
     * Send feedback via the Adaire webhook (SendGrid on our end, no key in plugin).
     *
     * @param string|array $recipient Feedback recipient email address(es).
     * @param string       $subject   Email subject (unused by the webhook, kept for parity).
     * @param string       $message   Plain-text email body (unused by the webhook, kept for parity).
     * @param array        $feedback  Sanitized feedback fields.
     * @return array
     */
    private function send_feedback_email($recipient, $subject, $message, array $feedback)
    {
        ...
        $sent = ! is_wp_error( $response ) && 200 === (int) wp_remote_retrieve_response_code( $response );
    ...
    /**
     * Store a log entry for troubleshooting.
     *
     * @param array        $feedback           Sanitized feedback fields.
     * @param array        $send_result        Result returned by send_feedback_email().
     * @param string|array $feedback_recipient  Recipient email address(es) the feedback was sent to.
     */
    ...
    /**
     * Get the public plugin URL (WordPress.org listing).
     *
     * @return string
     */
