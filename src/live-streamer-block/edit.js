  ...
  return (<>
    <InspectorControls>
      <PanelBody title={__('Content', 'adaire-blocks')} initialOpen={true}>
        ...
        <TextControl label={__('Stream URL', 'adaire-blocks')} value={a.streamUrl || ''} onChange={set(setAttributes, 'streamUrl')} /><TextareaControl label={__('Embed code', 'adaire-blocks')} value={a.embedCode || ''} onChange={set(setAttributes, 'embedCode')} /><SelectControl label={__('Platform', 'adaire-blocks')} value={a.platform || 'youtube'} options={[{label:'YouTube',value:'youtube'},{label:'Twitch',value:'twitch'},{label:'Vimeo',value:'vimeo'},{label:'Facebook',value:'facebook'},{label:'Instagram',value:'instagram'},{label:'TikTok',value:'tiktok'},{label:'Snapchat',value:'snapchat'},{label:'Custom',value:'custom'}]} onChange={set(setAttributes, 'platform')} /><ToggleControl label={__('Live now', 'adaire-blocks')} checked={!!a.isLive} onChange={set(setAttributes, 'isLive')} /><TextControl label={__('Next stream date/time', 'adaire-blocks')} value={a.nextStreamDate || ''} onChange={set(setAttributes, 'nextStreamDate')} /><ToggleControl label={__('Show chat', 'adaire-blocks')} checked={!!a.showChat} onChange={set(setAttributes, 'showChat')} />
      </PanelBody>
