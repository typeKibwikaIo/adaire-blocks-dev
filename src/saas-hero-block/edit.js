  ...
  return (<>
    <InspectorControls>
      ...
      <PanelBody title={__('Style', 'saas-hero-block')} initialOpen={false}>
        <AdaireColorControl label={__('Accent color', 'saas-hero-block')} value={a.accentColor} onChange={(v) => setAttributes({ accentColor: v || '#d52940' })} />
        <AdaireColorControl label={__('Background color', 'saas-hero-block')} value={a.backgroundColor} onChange={(v) => setAttributes({ backgroundColor: v || '#111827' })} />
        <AdaireColorControl label={__('Text color', 'saas-hero-block')} value={a.textColor} onChange={(v) => setAttributes({ textColor: v || '#ffffff' })} />
      ...
      <SelectControl label={__('Background type', 'saas-hero-block')} value={a.backgroundType || 'solid'} options={[{ label: 'Solid color', value: 'solid' }, { label: 'Gradient', value: 'gradient' }, { label: 'Image', value: 'image' }]} onChange={set(setAttributes, 'backgroundType')} />
        ...
        <AdaireColorControl label={__('Button hover text', 'saas-hero-block')} value={a.buttonHoverColor} onChange={(v) => setAttributes({ buttonHoverColor: v || '#ffffff' })} />
        <AdaireColorControl label={__('Button hover background', 'saas-hero-block')} value={a.buttonHoverBackgroundColor} onChange={(v) => setAttributes({ buttonHoverBackgroundColor: v || '#111827' })} />
      </PanelBody>
    </InspectorControls>
