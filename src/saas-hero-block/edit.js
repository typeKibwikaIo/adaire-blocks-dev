...
export default function Edit({ attributes, setAttributes, isSelected, clientId }) {
  ...
  // (see the identical fix in infogrid-2-block/edit.js for why this is needed).
  useEffect(() => {
    if (typeof document !== 'undefined' && !document.querySelector('[data-bi-saas-hero]')) {
      const link = document.createElement('link');
      link.rel = 'stylesheet';
      link.href = (window.AdaireBlocksPluginUrl || '') + 'assets/vendor/bootstrap-icons/bootstrap-icons.min.css';
      link.setAttribute('data-bi-saas-hero', '1');
