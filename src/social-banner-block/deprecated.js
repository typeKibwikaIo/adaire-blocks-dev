/**
 * Social Media Banner (social-banner-block) deprecations — most recent first.
 *
 * v1  Frozen copy of the save() that shipped before ADAB-010 added a Font
 *     Family control. This block has no text content of its own (it only
 *     renders linked icon glyphs), so ADAB-010's only change here is a
 *     `--banner-font-family` custom property added unconditionally to every
 *     instance's root style attribute. Posts saved before that change don't
 *     have that custom property in their stored markup, so re-running the
 *     *current* save() against them would produce a style attribute with an
 *     extra declaration that doesn't match what's stored, and Gutenberg
 *     would flag them as invalid content. No attribute schema changed shape
 *     (the new attribute is purely additive with a safe default that
 *     reproduces the original un-set/inherit font-family), so `migrate` is a
 *     no-op identity function and this entry doesn't need its own
