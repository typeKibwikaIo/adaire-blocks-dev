  ...
  return (<>
    <InspectorControls>
      <PanelBody title={__('Content', 'adaire-blocks')} initialOpen={true}>
        <TextControl label={__('Destination email', 'adaire-blocks')} value={a.destinationEmail || ''} onChange={set(setAttributes, 'destinationEmail')} />
        <TextControl label={__('Success message', 'adaire-blocks')} value={a.successMessage || ''} onChange={set(setAttributes, 'successMessage')} />
        <TextControl label={__('Redirect URL', 'adaire-blocks')} value={a.redirectUrl || ''} onChange={set(setAttributes, 'redirectUrl')} />
        <SelectControl label={__('Layout', 'adaire-blocks')} value={a.layout || 'two'} options={[{label:'1 column',value:'one'},{label:'2 columns',value:'two'}]} onChange={set(setAttributes, 'layout')} />
      </PanelBody>
      <PanelBody title={__('Form fields', 'adaire-blocks')} initialOpen={false}>
        {fields.map((field, index) => <div className="adaire-field-editor" key={index} style={{ borderTop: '1px solid #ddd', paddingTop: '12px', marginTop: '12px' }}>
          <TextControl label={__('Label', 'adaire-blocks')} value={field.label || ''} onChange={(value) => updateField(index, { label: value, name: field.name || normalizeName(value) })} />
          <TextControl label={__('Name', 'adaire-blocks')} value={field.name || ''} onChange={(value) => updateField(index, { name: value })} />
          <SelectControl label={__('Type', 'adaire-blocks')} value={field.type || 'text'} options={fieldTypes.map((type) => ({ label: type, value: type }))} onChange={(value) => updateField(index, { type: value })} />
          {field.type === 'select' && <TextareaControl label={__('Options, comma separated', 'adaire-blocks')} value={field.options || ''} onChange={(value) => updateField(index, { options: value })} />}
          <SelectControl label={__('Width', 'adaire-blocks')} value={field.width || 'half'} options={[{ label: 'Half', value: 'half' }, { label: 'Full', value: 'full' }]} onChange={(value) => updateField(index, { width: value })} />
          <SelectControl label={__('Required', 'adaire-blocks')} value={field.required ? 'yes' : 'no'} options={[{ label: 'Yes', value: 'yes' }, { label: 'No', value: 'no' }]} onChange={(value) => updateField(index, { required: value === 'yes' })} />
          <Button variant="secondary" isDestructive onClick={() => removeField(index)}>{__('Remove field', 'adaire-blocks')}</Button>
        </div>)}
        <Button variant="primary" onClick={addField}>{__('Add field', 'adaire-blocks')}</Button>
      </PanelBody>
      <PanelBody title={__('Button and background', 'adaire-blocks')} initialOpen={false}>
