 ...
 *
 * v3  Frozen copy of the save() that shipped before the "Content Width"
 *     control was added (2026-07-09). That change appends an
 *     `adaire-about--width-(contained|wide|full)` modifier class to the
 *     saved markup's className, and also adds `--ab-padding-top`/
 *     `--ab-padding-bottom` CSS custom properties to the inline style
 *     (used by style.scss's mobile media query to cap oversized desktop
 *     padding on small screens). Posts saved before this change have
 *     neither the extra class nor the extra custom properties, so
 *     re-running the *current* save() against them would no longer match
 *     the stored markup and Gutenberg would flag them as invalid. Same
 *     reasoning as v2/v1 below: purely additive, safe-default attributes,
 *     so `migrate` is a no-op identity function.
 *
