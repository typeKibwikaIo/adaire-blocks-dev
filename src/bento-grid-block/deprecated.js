 ...
 *
 * v4  Frozen copy of the save() that shipped before the
 *     `--infogrid4-grid-border-color` CSS custom property was renamed to
 *     `--infogrid4-grid-line-color`. The old name was a false-positive match
 *     for WordPress core's global styles `[style*=border-color] { border-style:
 *     solid }` rule (a plain substring match against the whole style
 *     attribute, not a real border-color property), which forced an unwanted
 *     solid border onto the block wrapper. Same `cards` attribute shape as
 *     current, so no `migrate` or explicit `attributes` override needed.
 *
...
export default [deprecatedV4, deprecatedV3, deprecatedV2, deprecatedV1];
