...
const buildTourSteps = ( blocks ) => {
	...
	const categorySteps = Object.entries( groupedBlocks ).map(
		( [ category, categoryBlocks ] ) => ( {
			...
			items: categoryBlocks.slice( 0, 8 ).map( ( block ) => ( {
				label: block.title,
				description: block.description || 'Use this block to build a reusable Adaire Blocks section.',
			} ) ),
		} )
	);
	...
	return [
		{
			title: 'Welcome to Adaire Blocks',
			description:
			...
			items: [
				{
					label: 'Add blocks',
					description:
						'Click the block inserter and search for Adaire Blocks by name or category.',
				},
				{
					label: 'Edit settings',
					description:
		...
		},
		{
			...
			description:
				'Use the Adaire Blocks responsive toolbar to preview Desktop, Tablet, Mobile, and Smartwatch layouts before saving.',
			items: [
				{
			...
			items: [
				{
					...
					description:
						'Use the Adaire Blocks Guide button in the editor toolbar whenever you want to review these steps again.',
				},
			],
		},
	];
...
function AdaireGetStartedTour() {
	...
	const toolbar = (
		<Button
			...
			onClick={ openTour }
			aria-label="Have a tour of Adaire Blocks"
		>
			Have a tour
		</Button>
	);
