  ...
  return (<>
    <InspectorControls>
      <PanelBody title={__('Content', 'adaire-blocks')} initialOpen={true}>
        ...
        <TextControl label={__('Expiry date/time', 'adaire-blocks')} value={a.expiryDate || ''} onChange={set(setAttributes, 'expiryDate')} /><ToggleControl label={__('Dismissible', 'adaire-blocks')} checked={!!a.dismissible} onChange={set(setAttributes, 'dismissible')} /><ToggleControl label={__('Countdown', 'adaire-blocks')} checked={!!a.countdown} onChange={set(setAttributes, 'countdown')} /><ToggleControl label={__('Sticky', 'adaire-blocks')} checked={!!a.sticky} onChange={set(setAttributes, 'sticky')} /><SelectControl label={__('Position', 'adaire-blocks')} value={a.position || 'top'} options={[{label:'Top',value:'top'},{label:'Bottom',value:'bottom'}]} onChange={set(setAttributes, 'position')} /><SelectControl label={__('Animation', 'adaire-blocks')} value={a.animationType || 'slide-in'} options={[{label:'Slide in',value:'slide-in'},{label:'Fade in',value:'fade-in'},{label:'None',value:'none'}]} onChange={set(setAttributes, 'animationType')} />
      </PanelBody>
      ...
      <PanelBody title={__('Responsive', 'adaire-blocks')} initialOpen={false}>
        ...
        <RangeControl label={__('Font size', 'adaire-blocks')} value={fontSize[deviceType] ?? fontSize.desktop ?? a.fontSize ?? 16} onChange={(v) => updateResponsive('responsiveFontSize', v)} min={10} max={80} />
        <RangeControl label={__('Padding', 'adaire-blocks')} value={padding[deviceType] ?? padding.desktop ?? a.padding ?? 28} onChange={(v) => updateResponsive('responsivePadding', v)} min={0} max={120} />
        <RangeControl label={__('Radius', 'adaire-blocks')} value={radius[deviceType] ?? radius.desktop ?? a.buttonRadius ?? 18} onChange={(v) => updateResponsive('responsiveBorderRadius', v)} min={0} max={80} />
      </PanelBody>
    </InspectorControls>
