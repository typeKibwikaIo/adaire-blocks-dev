...
export default function Edit({ attributes, setAttributes }) {
  ...
  return (<>
    <InspectorControls>
      <PanelBody title={__('Content', 'booking-form-block')} initialOpen={true}>
        <TextControl label={__('Destination email', 'booking-form-block')} value={a.destinationEmail || ''} onChange={set(setAttributes, 'destinationEmail')} />
        <TextControl label={__('Success message', 'booking-form-block')} value={a.successMessage || ''} onChange={set(setAttributes, 'successMessage')} />
        <TextControl label={__('Redirect URL', 'booking-form-block')} value={a.redirectUrl || ''} onChange={set(setAttributes, 'redirectUrl')} />
        <SelectControl label={__('Layout', 'booking-form-block')} value={a.layout || 'two'} options={[{label:'1 column',value:'one'},{label:'2 columns',value:'two'}]} onChange={set(setAttributes, 'layout')} />
      </PanelBody>
      <PanelBody title={__('Form fields', 'booking-form-block')} initialOpen={false}>
        {fields.map((field, index) => <div className="adaire-field-editor" key={index} style={{ borderTop: '1px solid #ddd', paddingTop: '12px', marginTop: '12px' }}>
          <TextControl label={__('Label', 'booking-form-block')} value={field.label || ''} onChange={(value) => updateField(index, { label: value, name: field.name || normalizeName(value) })} />
          <TextControl label={__('Name', 'booking-form-block')} value={field.name || ''} onChange={(value) => updateField(index, { name: value })} />
          <SelectControl label={__('Type', 'booking-form-block')} value={field.type || 'text'} options={fieldTypes.map((type) => ({ label: type, value: type }))} onChange={(value) => updateField(index, { type: value })} />
          {field.type === 'select' && <TextareaControl label={__('Options, comma separated', 'booking-form-block')} value={field.options || ''} onChange={(value) => updateField(index, { options: value })} />}
          <SelectControl label={__('Width', 'booking-form-block')} value={field.width || 'half'} options={[{ label: 'Half', value: 'half' }, { label: 'Full', value: 'full' }]} onChange={(value) => updateField(index, { width: value })} />
          <SelectControl label={__('Required', 'booking-form-block')} value={field.required ? 'yes' : 'no'} options={[{ label: 'Yes', value: 'yes' }, { label: 'No', value: 'no' }]} onChange={(value) => updateField(index, { required: value === 'yes' })} />
          <Button variant="secondary" isDestructive onClick={() => removeField(index)}>{__('Remove field', 'booking-form-block')}</Button>
        </div>)}
        <Button variant="primary" onClick={addField}>{__('Add field', 'booking-form-block')}</Button>
      </PanelBody>
      <PanelBody title={__('Button and background', 'booking-form-block')} initialOpen={false}>
        <SelectControl label={__('Background type', 'booking-form-block')} value={a.backgroundType || 'solid'} options={[{ label: 'Solid color', value: 'solid' }, { label: 'Gradient', value: 'gradient' }, { label: 'Image', value: 'image' }]} onChange={set(setAttributes, 'backgroundType')} />
        {a.backgroundType === 'image' && media(__('Background image', 'booking-form-block'), a.backgroundImage, (v) => setAttributes({ backgroundImage: v }))}
        {a.backgroundType === 'gradient' && <TextControl label={__('Gradient CSS', 'booking-form-block')} value={a.backgroundGradient || a.gradient || ''} onChange={set(setAttributes, 'backgroundGradient')} />}
