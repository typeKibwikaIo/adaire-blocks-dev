...
/**
 * Testimonial (testimonial-block) deprecations — most recent first.
 *
 * v3  Frozen copy of the save() that shipped before ADAB-010 added full
 *     typography override controls (font weight/line height/letter
 *     spacing/text transform for the quote, author name, author title, and
 *     company name text roles, plus a block-level font family) —
 *     unconditionally, for every instance, regardless of whether the user
 *     ever opens the new role-specific Typography controls. Posts saved
 *     before that change don't have these CSS custom properties on this
 *     block's saved markup `style` attribute at all, so re-running the
 *     *current* save() against them would produce a style attribute that
 *     doesn't match what's stored, and Gutenberg would flag them as invalid
 *     content. No attribute schema changed shape (the new attributes are
 *     purely additive with safe defaults that reproduce the original
 *     hardcoded values), so `migrate` is a no-op identity function and this
 ...
 *     that omits one).
 *
 * v2/v1 (below, pre-existing) Legacy "ad-carousel-text-block" history —
 *     frozen save() from before the responsive padding-top/padding-bottom
 *     CSS vars were introduced. Logic preserved exactly as it was; do not
 *     alter.
 */
const deprecatedV3 = {
    migrate(attributes) {
