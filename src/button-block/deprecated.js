 ...
 *
 * v0  Oldest known shape, predating even v1 — see its own comment further
 *     below, right above its definition, for details.
 */
...
/**
 * v0  Oldest known save() shape — predates this repo's git history entirely
 *     (even deprecatedV1 already has full responsive font-size/padding/
 *     margin CSS custom properties; this one doesn't). Reconstructed
 *     directly from a live block-validation error's "Content retrieved from
 *     post body" dump (2026-07-13, "Back to all blocks" button) rather than
 *     from source history, since no earlier commit/version in this repo
 *     matches it. Distinguishing features versus v1: no
 *     `--button-font-size-{tablet,mobile,watch}` / `--button-padding-*-
 *     {tablet,mobile,watch}` / `--button-margin-*-{tablet,mobile,watch}`
 *     variables at all (desktop-only values), no `--button-line-height*`,
 *     `--button-letter-spacing*`, `--button-text-transform`, or
 *     `--button-font-family` variables, no `adaire-button-block__link--icon-
 *     {position}` class on the `<a>`, and the icon `<svg>` is a bare sibling
 *     inside the `<a>` rather than wrapped in a `<span class="…__icon">`.
 *     `migrate` is a no-op: nothing about the *attribute schema* changed
 *     shape, only how many of the already-existing (or since-added,
 *     safely-defaulted) attributes the save() template rendered into CSS
 *     variables/markup.
 */
const deprecatedV0 = {
  migrate(attributes) {
...
export default [deprecatedV3, deprecatedV2, deprecatedV1, deprecatedV0];
