/**
 * Hero 1 deprecations - most recent first.
 *
 * v1  Frozen copy of the save() that shipped before the per-tier hard-coded
 *     defaults (min-height auto/100vh, cta width 60%/45%, 200px/80px padding,
 *     72px/52px heading, 700px/500px icon, 100vw container width) were
 *     replaced with the shared fluid clamp() values in fluid-defaults.js.
 *     Those defaults are written into the saved inline style attribute, so
 *     without this entry every previously saved Hero 1 block fails validation.
 *     Attribute shape is unchanged, so migrate() is the identity.
 */
import { useBlockProps, useInnerBlocksProps } from '@wordpress/block-editor';
import { __ } from '@wordpress/i18n';

function deprecatedV1Save({ attributes }) {
    const {
        blockId,
        breadcrumbsText,
        breadcrumbsItems,
