/**
 * Footer block is now a dynamic block: frontend markup is produced by
 * src/website-footer-block/render.php (registered via block.json's
 * "render" field), not by this save(). Returning null here is required for
 * dynamic blocks and means the block comment saved in post_content carries
 * no markup — WordPress calls the PHP render callback on every request, so
 * content (e.g. live WP menu data in nav-type columns, widget-area output)
