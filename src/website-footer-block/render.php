...
if ( ! function_exists( 'adaire_footer_render_top_bar' ) ) {
	...
	function adaire_footer_render_top_bar( $attributes ) {
		...
		if ( ! empty( $top_bar['showSocialMedia'] ) ) {
			...
			$out  .= '<div class="website-footer-block__top-bar-social">';
			foreach ( $links as $link ) {
				$label = isset( $link['label'] ) ? $link['label'] : '';
				$url   = ! empty( $link['url'] ) ? $link['url'] : '#';
				$out  .= '<div class="website-footer-block__social-link-wrapper">';
				if ( isset( $link['displayStyle'] ) && 'icon' === $link['displayStyle'] ) {
					$out .= '<a href="' . esc_url( $url ) . '" class="website-footer-block__social-icon" aria-label="' . esc_attr( wp_strip_all_tags( $label ) ) . '" target="_blank" rel="noopener noreferrer">' . adaire_footer_icon_svg( isset( $link['platform'] ) ? $link['platform'] : '' ) . '</a>';
				} else {
...
if ( ! function_exists( 'adaire_footer_render_bottom_bar' ) ) {
	...
	function adaire_footer_render_bottom_bar( $attributes ) {
		...
		if ( ! empty( $bottom_bar['showSocialIcons'] ) ) {
			...
			$out         .= '<div class="website-footer-block__bottom-bar-social">';
			foreach ( $social_icons as $link ) {
				$label = isset( $link['label'] ) ? $link['label'] : '';
				$url   = ! empty( $link['url'] ) ? $link['url'] : '#';
				$out  .= '<a href="' . esc_url( $url ) . '" class="website-footer-block__social-icon" aria-label="' . esc_attr( wp_strip_all_tags( $label ) ) . '" target="_blank" rel="noopener noreferrer">' . adaire_footer_icon_svg( isset( $link['icon'] ) ? $link['icon'] : '' ) . '</a>';
			}
