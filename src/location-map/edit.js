...
export default function Edit({ attributes, setAttributes, clientId }) {
    ...
    useEffect(() => {
        const blockConfig = window.adaireBlocksConfig?.blocks?.['location-map'] || {};
    ...
    return (
        <>
            <InspectorTabs attributes={attributes} setAttributes={setAttributes}>
                {/* Locations Panel */}
                <PanelBody section="content" title={__('Locations', 'ad-map')} initialOpen={true}>
                    {(locations || []).map((loc, i) => (
                ...
                <PanelBody section="layout" title={__('Container Settings', 'ad-map')} initialOpen={true}>
                ...
                {/* Behavior */}
                <PanelBody section="layout" title={__('Behavior', 'ad-map')} initialOpen={false}>
                    {(() => {
                ...
                {/* Margins Panel */}
                <PanelBody section="layout" title={__('Margins', 'ad-map')} initialOpen={false}>
                    <p style={{ marginBottom: '8px', fontWeight: 600 }}>
                        {__('Device', 'ad-map')}
                    </p>
                    <ButtonGroup style={{ marginBottom: '12px' }}>
                ...
                {/* Padding Panel */}
                <PanelBody section="layout" title={__('Padding', 'ad-map')} initialOpen={false}>
                    <p style={{ marginBottom: '8px', fontWeight: 600 }}>
                        {__('Device', 'ad-map')}
                    </p>
                    <ButtonGroup style={{ marginBottom: '12px' }}>
                ...
                {/* Appearance */}
                <PanelBody section="style" priority="high" title={__('Appearance', 'ad-map')} initialOpen={false}>
                    <div>
                        <AdaireColorControl
                            label={__('List Text', 'ad-map')}
                            value={navTextColor}
                            onChange={(v) => setAttributes({ navTextColor: v })}
                        />
                        <AdaireColorControl
                            label={__('Active Country Background', 'ad-map')}
                            value={navActiveBgColor}
                            onChange={(v) => setAttributes({ navActiveBgColor: v })}
                        />
                        <AdaireColorControl
                            label={__('Active Text', 'ad-map')}
                            value={navActiveTextColor}
                            onChange={(v) => setAttributes({ navActiveTextColor: v })}
                        />
                    </div>
                    <div style={{ marginTop: 12 }}>
                        <SelectControl
                        ...
                        <div className="ad-map__address">
                            {locations.map((loc, i) => (
                                <div key={loc.id} className={`ad-map__address__panel ${i === (activeIndex ?? 0) ? 'is-active' : ''}`}>
                                    <QuickZone
                                        id={`contact-${loc.id}`}
                                        label={__('Contact Details', 'ad-map')}
                        ...
                        <div className="ad-map__map">
                            {singleMapMode ? (
                                <div className="ad-map__map__panel is-active">
                                    <QuickZone
                                        id={`map-${locations?.[activeIndex ?? 0]?.id ?? 'single'}`}
                                        label={__('Map Embed', 'ad-map')}
                                        activeZone={activeZone}
                                        setActiveZone={setActiveZone}
                                        content={
                                            <TextControl
                                                label={__('Map Embed URL', 'ad-map')}
                                                value={locations?.[activeIndex ?? 0]?.mapEmbedUrl || ''}
                                                onChange={(v) => {
                                                    const idx = activeIndex ?? 0;
                                                    const next = [...locations];
                                                    next[idx] = { ...next[idx], mapEmbedUrl: v };
                                                    setAttributes({ locations: next });
                                                }}
                                            />
                                        }
