...
class AdaireBlocksLicense {
	...
	public function activate_license( $license_key ) {
		...
		if ( ! isset( $data['success'] ) ) {
			...
			return array(
				'success' => false,
				'message' => 'Invalid response format from license server',
			);
		}
		...
		if ( isset( $lmfwc_response['success'] ) && ! $lmfwc_response['success'] ) {
			$lmfwc_errors = $activation_data['errors'] ?? $lmfwc_response['errors'] ?? array();
			if ( isset( $lmfwc_errors['lmfwc_rest_data_error'] ) ) {
				return array(
					'success' => false,
					'message' => $lmfwc_errors['lmfwc_rest_data_error'][0],
				);
			}
