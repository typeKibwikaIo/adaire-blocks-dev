...
function startMigration() {
    if (!confirm('Are you sure you want to start the migration? This will update all posts, pages, and patterns with AdaireBlocks Blocks.')) {
        return;
    }
    ...
    .then(data => {
        if (data.success) {
            ...
            addLog(`   - Posts/Pages with AdaireBlocks Blocks: ${postsCount}`);
            addLog(`   - Patterns with AdaireBlocks Blocks: ${patternsCount}`);
            ...
            if (postsCount > 0 && patternsCount > 0) {
                ...
                addLog(`ℹ️ No items found with AdaireBlocks Blocks to migrate`);
            }
