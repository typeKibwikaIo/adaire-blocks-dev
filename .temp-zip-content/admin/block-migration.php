...
/**
 * Add migration submenu to AdaireBlocks Blocks admin menu
 */
function adaire_blocks_add_migration_menu() {
    add_submenu_page(
        'adaire-blocks-settings',  // Parent slug (AdaireBlocks Blocks menu)
...
function adaire_blocks_migration_page() {
    ...
    ?>
    <div class="wrap">
        <h1>AdaireBlocks Blocks Migration Tool</h1>
        
        <div class="card" style="max-width: 800px; margin-top: 20px;">
            <h2> Update All Blocks (Queue-Based Migration)</h2>
            <p>
                This tool uses a fast queue-based system to find all posts, pages, and reusable block patterns that contain AdaireBlocks Blocks 
            ...
            <ul>
                <li> Finds all posts/pages and patterns with AdaireBlocks Blocks</li>
            ...
            <p style="margin-top: 20px; font-size: 13px; color: #666;">
                <strong>Note:</strong> This tool will process both posts/pages and reusable block patterns (wp_block) that contain AdaireBlocks Blocks.
            </p>
        </div>
    </div>
    <?php
...
function adaire_get_posts_to_migrate() {
    ...
    // Filter posts that contain AdaireBlocks Blocks
    foreach ($all_posts as $post_id) {
        $content = get_post_field('post_content', $post_id);
        
        // Check if post contains any AdaireBlocks Blocks
    ...
    // Filter patterns that contain AdaireBlocks Blocks
    foreach ($all_patterns as $pattern_id) {
        $content = get_post_field('post_content', $pattern_id);
        
        // Check if pattern contains any AdaireBlocks Blocks
        if (has_blocks($content)) {
            $blocks = parse_blocks($content);
            if (adaire_has_adaire_blocks($blocks)) {
...
/**
 * Check if blocks array contains any AdaireBlocks Blocks
 */
function adaire_has_adaire_blocks($blocks) {
    foreach ($blocks as $block) {
        // Check if it's an AdaireBlocks Block (starts with 'create-block/')
