...
function adaire_blocks_send_via_sendgrid($to, $subject, $message, $reply_to = null)
{
    ...
    $from_name = apply_filters('adaire_blocks_deactivation_feedback_from_name', 'AdaireBlocks Blocks', []);

    $payload = [
        'personalizations' => [
            [
                'to' => is_array($to) ? array_map(function($t) { return ['email' => $t]; }, $to) : [['email' => $to]],
                'subject' => $subject,
            ],
        ],
        'from' => [
