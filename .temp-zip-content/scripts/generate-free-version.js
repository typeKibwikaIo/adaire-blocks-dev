...
/**
 * Free Version Generator for AdaireBlocks Blocks
 *
...
class FreeVersionGenerator {
    ...
    async generatePackageJson() {
        ...
        const freePackage = {
            ...
            description: 'Free version of AdaireBlocks Blocks - Professional WordPress blocks for Gutenberg editor',
            main: 'src/index.js',
            scripts: {
                prebuild: 'node scripts/apply-new-icons.js && node scripts/update-block-icons.js',
                build: 'node --max-old-space-size=8192 node_modules/@wordpress/scripts/bin/wp-scripts.js build',
    ...
    async buildFreeVersion() {
        ...
        try {
            ...
            try {
                ...
                execSync(
                    `node --max-old-space-size=8192 "${wpScriptsBin}" build`,
    ...
    modifyIndexJs() {
        ...
        let indexContent = `// AdaireBlocks Blocks Free Version - auto-generated, do not edit manually\n\n`;
        enabledBlocks.forEach(blockName => {
    ...
    async createZipFile() {
        ...
        try {
            ...
            // Ensure plugin-zips directory exists
            ...
            // Change to free version directory and create zip
            const originalDir = process.cwd();
            process.chdir(this.freeVersionDir);

            // Remove build directory before zipping (not needed in distribution)
            const buildDir = path.join(this.freeVersionDir, 'build');
            if (fs.existsSync(buildDir)) {
                fs.rmSync(buildDir, { recursive: true, force: true });
                console.log('   ✓ Build directory removed for distribution');
            }

            // Remove node_modules directory before zipping (not needed in distribution)
            ...
            // Create zip manually with correct structure using PowerShell
            // WordPress expects plugin files to be in the root of the zip, not in a subdirectory
            const tempZipPath = path.join(this.freeVersionDir, 'adaire-blocks-free-temp.zip');
            
            // Use PowerShell to create the zip with all files except excluded ones
            const excludeDirs = ['node_modules', 'build', '.git'];
            const excludeFiles = ['.gitignore', 'package.json', 'package-lock.json'];
            ...
            getAllFiles(this.freeVersionDir);
            
            // Create a temporary directory structure for zipping
            const tempDir = path.join(this.sourceDir, '.temp-zip-content');
            ...
            filesToInclude.forEach(file => {
                const srcPath = path.join(this.freeVersionDir, file);
            ...
            console.log(`   ✓ Prepared ${filesToInclude.length} files for zipping`);
            ...
            // Move the created zip to plugin-zips directory
            ...
            console.log('   You can manually create the zip by running: npm run plugin-zip in the free version directory');
        }
    }
