...
if (fs.existsSync(mappingPath)) {
    BLOCK_ICON_MAP = JSON.parse(fs.readFileSync(mappingPath, 'utf8')).blockIconMap || {};
...
function updateBlockIcons() {
    ...
    blockDirs.forEach(blockName => {
        ...
        try {
            ...
            // Each entry is { component, svg? } — fall back to stripping -block suffix
            const iconName = (BLOCK_ICON_MAP[blockName] && BLOCK_ICON_MAP[blockName].component) || blockName.replace('-block', '');
